...
class Curl
{
     ...
     * @access public
     */
    public function Curl($debug = false)
    {
        $this->debug = $debug;
     ...
     * @access public
     */
    public function send_post_data($url, $postdata, $arrayHeader = null, $ip = null, $timeout = 600)
    {
     ...
     * @access public
     */
    public function fetch_url($url, $getdata = null, $ip = null, $timeout = 600)
    {
        if ($this->debug) {
            curl_setopt($this->ch, CURLOPT_VERBOSE, true);
        }
    ...
    public function send_post_content($url, $headersdata, $postdata, $content, $ip = null, $timeout = 10)
    {
        if ($this->debug) {
            curl_setopt($this->ch, CURLOPT_VERBOSE, true);
        }
    ...
    public function send_put_data($url, $headersdata, $putdata, $ip = null, $timeout = 10)
    {
        if ($this->debug) {
            curl_setopt($this->ch, CURLOPT_VERBOSE, true);
        }
    ...
    public function send_delete($url, $ip = null, $timeout = 10)
    {
        if ($this->debug) {
            curl_setopt($this->ch, CURLOPT_VERBOSE, true);
        }
